...
There were 5 pairs at each complexity level, so 15 pairs per session total. Pairs within the same complexity level did not all show the same attributes. One simple pair might show price, image quality, and battery life, while another showed weight, ease of use, and manual controls. This ensured no single trade-off dominated the design and that participants had to think carefully about different combinations of features at every level. Figure~\ref{fig:camera-pair} shows an example simple pair as it was presented.

\begin{figure}[h]
  \centering
  \includegraphics[width=\columnwidth]{camera-pair-example.png}
  \caption{Example simple-condition camera pair (3 visible attributes). Both cameras have a clear strength: Alvex T1 wins on price and battery life, Brikon S2 wins on image quality. There is no dominant option; the participant's choice depends on which attribute they ranked highest in the priority elicitation step.}~\label{fig:camera-pair}
\end{figure}
