...
\end{equation}
For LLM personas, persona-level scores are the mean of the 10 runs; the group score for label $L \in \{\text{expert}, \text{novice}\}$ at complexity $\ell$ is the mean across the 7 personas in that label. Because each $\mathrm{VCA}_{\text{pair}} \in [0,1]$ as a weighted average of indicators, all aggregates lie in $[0,1]$ as well, where 1 indicates perfect alignment between stated priorities and observed choices and 0 indicates that every choice contradicted the stated ranking. We deliberately use a graded $[0,1]$ score rather than a binary top-attribute hit rate so that partial alignment is captured: a choice that satisfies the participant's 2nd- and 3rd-ranked priorities but not the 1st is meaningfully different from a choice that satisfies none of them, and a $[0,1]$ scale preserves that distinction while a 0/1 indicator discards it.
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.6\linewidth]{vca-by-complexity.png}
...
\begin{table}[h]
  \caption{Mean value-choice alignment by population, label, and complexity.}
  \label{tab:vca}
  \centering
